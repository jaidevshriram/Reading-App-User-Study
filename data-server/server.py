...
def get_song_list(chap_num):
    songs = sorted(os.listdir(f"../../SoundtrackingBooks-Streamlit/generated_chunked/{chap_num}"))
    # songs = list(map(lambda x: os.path.join(f"../../SoundtrackingBooks-Streamlit/generated_chunked/{chap_num}", x), songs))
    songs = list(map(lambda x: os.path.join(f"http://localhost:5000/generated_chunked/{chap_num}", x), songs))
